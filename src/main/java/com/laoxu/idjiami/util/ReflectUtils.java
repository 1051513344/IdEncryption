...
import com.laoxu.idjiami.annotation.EmojiParse;
...
import com.laoxu.idjiami.exception.UnsupportedTargetException;

/**
 * Created by dehua.lai on 2017/7/18.
 */
public abstract class ReflectUtils {
     ...
     * look{@link ReflectUtils#copyProperties(java.lang.Object, java.lang.Class, cn.ucmed.yilian.common.utils.ReflectUtils.TriFunction)}
     * @param source
     * @param tClass
     * @param <S>
     * @param <T>
    ...
    public static List<Field> getObjectAttribute(Object object){
        if(null == object) return Lists.newArrayList();
        //拿到该类
    ...
    //通过反射来修改表情的入库和出库
    public static <T> T parseEmoji(Object object){
        if(null == object) return null;
        List<Field> fieldList = getObjectAttribute(object);
        if(fieldList.isEmpty()) return null;
        for(Field field : fieldList){
            boolean fieldHasAnno = field.isAnnotationPresent(EmojiParse.class);
            if(fieldHasAnno){
                EmojiParse fieldAnno = field.getAnnotation(EmojiParse.class);
                int type = fieldAnno.type();
                Object value = getFieldForce(field,object);
                if(null == value) continue;
                if(1 == type){
                    //设置成员变量的值
                    setFieldForce(field.getName(),object,EmojiFilter.emojiConvert1(value.toString()));
                }
                if(2 == type){
                    //设置成员变量的值
                    setFieldForce(field.getName(),object,EmojiFilter.emojiRecovery2(value.toString()));
                }
            }
        }
        return (T)object;
